...
class AIMessage:
    ...
    @classmethod
    def create_system_message(cls, content: Union[str, Sequence[AIMessageContent]]) -> "AIMessage":
        """Factory method to create system messages"""
        if isinstance(content, str):
            return cls("system", [AIMessageContentFactory.create_text(content)])
        return cls("system", list(content))
