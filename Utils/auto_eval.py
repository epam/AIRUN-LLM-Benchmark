...
def evaluate(model: Model, language: str = "JS", force_reevaluate: bool = False):
    """
        ...
        for root, dirs, files in os.walk(base_path / experiment_type):
            if category_name in dirs:
                ...
                for evaluation_model in evaluation_models:
                    ...
                    summary_report.at[index, accuracy_cell_model_name] = round(accuracy_grading.get_score(), 2)
                    summary_report.at[index, completeness_cell_model_name] = round(completeness_grading.get_score(), 2)

                    # add the detailed results to the grading report
                    grading_report.append(
                        {
                            ...
                            "Category": category,
                            "Evaluator": evaluation_model.name,
                            "Accuracy": round(accuracy_grading.get_score(), 2),
                            "Completeness": round(completeness_grading.get_score(), 2),
                        }
                    )
